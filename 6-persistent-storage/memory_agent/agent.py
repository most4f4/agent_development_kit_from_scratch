...
# ToolContexts provide access to the session state for persistent storage.
# for example, tool_context.state is a dict that persists across interactions in the same session.
# to get or set values in the state, use value = tool_context.state.get('key', default_value) and tool_context.state['key'] = value.
# This allows the agent to remember information across conversations.
# function tools that interact with the persistent state require a ToolContext parameter.
