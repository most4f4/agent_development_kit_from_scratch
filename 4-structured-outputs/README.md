...
1. **Pydantic Model**: The `CapitalOutput` class defines the expected output structure, ensuring the agent's response includes a "capital" field with a string value.
2. **Instruction**: The agent's instruction explicitly states the required response format, guiding the LLM to produce the correct JSON structure.
3. **Output Key**: The `output_key` parameter stores the result in the session state, making it accessible for future use by other agents.
...
│   └── __init__.py                # Pydantic models for structured output
│
